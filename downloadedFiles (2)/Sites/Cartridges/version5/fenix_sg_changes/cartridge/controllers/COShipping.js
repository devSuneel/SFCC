'use strict';
/*global FenixIntelligentDeliveryUtils, app, guard*/
/*eslint no-restricted-globals: ["FenixIntelligentDeliveryUtils", "app", "guard"]*/
...
function selectShippingMethod() {
    ...
    if(dw.system.Site.current.preferences.custom.fenixEnabled) {    
	    ...
	    if(!FenixIntelligentDeliveryUtils.isEmpty(request.httpParameterMap.postalCode.submitted)){
			deliveryPostalCode = request.httpParameterMap.postalCode.submitted;
		}else if(!FenixIntelligentDeliveryUtils.isEmpty(address.postalCode)){
			deliveryPostalCode = address.postalCode;
		}else if(!FenixIntelligentDeliveryUtils.isEmpty(session.privacy.f_postal_code)){
			deliveryPostalCode = session.privacy.f_postal_code;
		}else if(customer.authenticated){
			deliveryPostalCode = !FenixIntelligentDeliveryUtils.isEmpty(customer.profile.addressBook.preferredAddress) ? (customer.profile.addressBook.preferredAddress.postalCode) : (pdict.CurrentCustomer.profile.addressBook.addresses[0].postalCode);
		}else{
		...
		if(isFenixIntelligentDeliveryEnabled) {
			...
			FenixIntelligentDeliveryGetResponse = FenixIntelligentDeliveryUtils.getDeliveryResonseObject(FenixIntelligentDeliveryUtils.getEstimates(cart.object, null, deliveryPostalCode, Resource.msg('FenixIntelligentDelivery.CheckOut','fenixIntelligentDelivery','COP')));         
						
			applicableShippingMethods = FenixIntelligentDeliveryUtils.getApplicableShippingMethods(applicableShippingMethods, FenixIntelligentDeliveryGetResponse);
    ...
    app.getView({
        Basket: cart.object,
        FenixIntelligentDelivery: FenixIntelligentDeliveryGetResponse
...
function updateShippingMethodList() {
	...
	/**
	 * Delivery Estimates section.  Set applicableShippingMethods to the shipping methods returned from the Delivery Estimates server call.
	 * 
	 */
    var FenixIntelligentDeliveryGetResponse;
    ...
    if(dw.system.Site.current.preferences.custom.fenixEnabled) {   
	    ...
	    var Resource = require('dw/web/Resource');    
	    var isFenixIntelligentDeliveryEnabled = FenixIntelligentDeliveryUtils.isFenixIntelligentDeliveryEnabled(address.countryCode?address.countryCode:'');
	    
	    var deliveryPostalCode;
	    
		
		if(!FenixIntelligentDeliveryUtils.isEmpty(address.postalCode)){
			deliveryPostalCode = address.postalCode;
		}else if(!FenixIntelligentDeliveryUtils.isEmpty(session.privacy.f_postal_code)){
			deliveryPostalCode = session.privacy.f_postal_code;
		}else if(customer.authenticated){
		...
		/**
		 * Setting the fenixHideShipID preference with a shipping ID, here we hide that shipping method.
		 */
		var	hideShipID	= dw.system.Site.current.preferences.custom.fenixHideShipID?dw.system.Site.current.preferences.custom.fenixHideShipID:'';
		
		if(!FenixIntelligentDeliveryUtils.isEmpty(hideShipID)){
			var shippingmethods = new ArrayList();
			
			for (var i = 0; i < applicableShippingMethods.length; i++) {
				if (applicableShippingMethods[i].ID !== hideShipID) {
					shippingmethods.add(applicableShippingMethods[i]);		
				}
			}
