...
function newDirectory(file, directoryName) {
	if (!file.directory) {
		...
		var newDirectory = new File(newDirectoryPath);
...
function getSFTP(action, url, remotePath, user, password, directory) {
	...
	var path = remotePath ? remotePath : '.';
	...
	try {
		...
		if (empty(fileInfo)) {
			...
			var fileName = [directory.fullPath, fileName].join(File.SEPARATOR);
...
function getWebDAV(action, url, path, user, password, directory) {
	...
	var webDAVClient;
	var remoteFolderURL = (url + (path ? path : '/')).replace(/\s/g, '%20');
