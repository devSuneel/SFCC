...
server.post('PlaceOrder', server.middleware.https, function (req, res, next) {
    ...
    var coCustomerForm = server.forms.getForm('coCustomer');
    var isOptIn = form.shippingAddress.isOptin.checked;
    ...
    if(coCustomerForm.email.htmlValue){
        Transaction.wrap(function () {
            var CustomerMgr = require('dw/customer/CustomerMgr');
            var password = 'Sfra@12345';
            var newCustomer = CustomerMgr.createCustomer(coCustomerForm.email.htmlValue,password);
            newCustomer.getProfile().email = coCustomerForm.email.htmlValue;
            newCustomer.getProfile().custom.OptIn = isOptIn;  
        });
    }
    else{
        // Adding the opt-in value to profile
        Transaction.wrap(function () {
            customer.getProfile().custom.OptIn = isOptIn;
        });
    }
